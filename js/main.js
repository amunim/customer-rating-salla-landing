jQuery(document).ready(function () {
  ...
  jQuery(".tbdemo-circle-icon-content .tbdemo-edit-button").parent().on("click", function() {
  ...
  jQuery(".tbdemo-switch, .tbdemo-plans_tab").on("click", function() {
    jQuery(".tbdemo-plans_tab_active").removeClass("tbdemo-plans_tab_active");
    let url = jQuery(".tbdemo-upgrade-price-button").attr("href");
    if( jQuery(this).hasClass("tbdemo-active_monthly") || jQuery(this).hasClass("tbdemo-plan_annual") ) {
        jQuery(".tbdemo-switch").removeClass("tbdemo-active_monthly").addClass("tbdemo-active_annual");
        jQuery(".tbdemo-plans_tab.tbdemo-plan_annual").addClass("tbdemo-plans_tab_active");
        let agency_price = jQuery(".tbdemo-upgrade-price-amount-agency").data('annual');
        let personal_price = jQuery(".tbdemo-upgrade-price-amount-personal").data('annual');
        jQuery(".tbdemo-upgrade-price-amount-personal").text(personal_price);
        jQuery(".tbdemo-upgrade-price-amount-agency").text(agency_price);
        jQuery(".tbdemo-upgrade-price-info").text(taa.billed_annually);

        let new_url = tbdemo_change_url_param( url, 'subscr_id', 165 );
        jQuery(".tbdemo-upgrade-personal .tbdemo-upgrade-price-button, .tbdemo-upgrade-personal .tbdemo-upgrade-price-button-mobile").attr("href", new_url);
        new_url = tbdemo_change_url_param( url, 'subscr_id', 173 );
        jQuery(".tbdemo-upgrade-agency .tbdemo-upgrade-price-button, .tbdemo-upgrade-agency .tbdemo-upgrade-price-button-mobile").attr("href", new_url);
    } else {
        jQuery(".tbdemo-switch").removeClass("tbdemo-active_annual").addClass("tbdemo-active_monthly");
        jQuery(".tbdemo-plans_tab.tbdemo-plan_monthly").addClass("tbdemo-plans_tab_active");
        let agency_price = jQuery(".tbdemo-upgrade-price-amount-agency").data('monthly');
        let personal_price = jQuery(".tbdemo-upgrade-price-amount-personal").data('monthly');
        jQuery(".tbdemo-upgrade-price-amount-personal").text(personal_price);
        jQuery(".tbdemo-upgrade-price-amount-agency").text(agency_price);
        jQuery(".tbdemo-upgrade-price-info").text(taa.billed_monthly);

        let new_url = tbdemo_change_url_param( url, 'subscr_id', 163 );
        jQuery(".tbdemo-upgrade-personal .tbdemo-upgrade-price-button, .tbdemo-upgrade-personal .tbdemo-upgrade-price-button-mobile").attr("href", new_url);
        new_url = tbdemo_change_url_param( url, 'subscr_id', 171 );
        jQuery(".tbdemo-upgrade-agency .tbdemo-upgrade-price-button, .tbdemo-upgrade-agency .tbdemo-upgrade-price-button-mobile").attr("href", new_url);
    }
  });

  jQuery(".twbb-pu-upgrade-layout, .tbdemo-upgrade-layout, .tbdemo-upgrade-close, .twbb-pu-upgrade-close").on("click", function() {
    jQuery(".tbdemo-upgrade-layout, .tbdemo-upgrade-container").hide();
...
function tbdemo_show_upgrade_popup() {
  let abtest = tbdemo_getCookie("db-ab-demo-testing") ? true : false;
  if ( abtest ) {
    jQuery(".tbdemo-upgrade-layout, .tbdemo-upgrade-container").removeAttr("style");
  } else {
